...
public:
	...
	void rotate(float angleRadians, glm::vec3 axis) {
		//front = glm::rotate(front, angleRadians, axis);
		front = glm::normalize(glm::vec3(MyGLM::rotate3D(axis, angleRadians) * glm::vec4(front, 1.0f)));
	...
	void setBillboard(const glm::vec3& camRight,
